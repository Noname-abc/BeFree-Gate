...
def sample_detection_observation(position, candidates, true_index):
    """
...
def correct_object_belief(belief_pred, candidates, true_index, position):
    """
    Bayesian correction step for one object class.
        b_k(z) proportional to q_y(y_k | z, position) * b_pred(z)
    """
    observation = sample_detection_observation(position, candidates, true_index)
...
def expected_target_position(
    ...
    if not carrying_key:
        return np.sum(key_candidates * b_key[:, None], axis=0)
    return np.sum(door_candidates * b_door[:, None], axis=0)
...
def grid_neighbors(ix, iy):
    return [
        (min(ix + 1, N_X_BINS - 1), iy),
        (max(ix - 1, 0), iy),
        (ix, min(iy + 1, N_Y_BINS - 1)),
        (ix, max(iy - 1, 0)),
    ]
...
def sample_omega(p_action, deterministic=False):
    if deterministic:
        idx = int(np.argmax(p_action))
    else:
        idx = np.random.choice(N_ACTIONS, p=p_action)
...
def sample_robotarium_episode_configs(n_episodes, seed=10000):
...
def run_robotarium_configs(
    ...
    with open(os.path.join(RESULTS_DIR, f"{prefix}_all_results.pkl"), "wb") as f:
        pickle.dump(stats, f)
    ...
    np.savez_compressed(
        ...
        key_visible_time=np.array(key_times),
